<template>
    <div class="bg-primary w-full p-8 pt-20 pb-20 flex">
        <div class="flex gap-2 md:gap-10  font-pepe text-2xl" style="margin:0 auto;align-items: center;">
        <RouterLink to="/#top" @mouseover="playClick"><img src="../../logo.png" class="nav-logo inline hover:scale-110 hover:skew-x-6"></RouterLink>
        <RouterLink to="/#top" @mouseover="playClick" class="hover:hover:scale-125 hover:skew-x-6 hidden md:block"> Home</RouterLink>
        <RouterLink to="/dashboard" @mouseover="playClick" class="hover:hover:scale-125 hover:skew-x-6"> Dashboard</RouterLink>
        <RouterLink to="/arena" @mouseover="playClick" class="hover:scale-125 hover:skew-x-6"> Arena</RouterLink>
        <a href="" @mouseover="playClick" class="hover:scale-125 hover:skew-x-6"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 inline">
        <path d="M14 1.75a.75.75 0 0 0-.89-.737l-7.502 1.43a.75.75 0 0 0-.61.736v2.5c0 .018 0 .036.002.054V9.73a1 1 0 0 1-.813.983l-.58.11a1.978 1.978 0 0 0 .741 3.886l.603-.115c.9-.171 1.55-.957 1.55-1.873v-1.543l-.001-.043V6.3l6-1.143v3.146a1 1 0 0 1-.813.982l-.584.111a1.978 1.978 0 0 0 .74 3.886l.326-.062A2.252 2.252 0 0 0 14 11.007V1.75Z" />
        </svg> Music on</a>
        <a href="https://twitter.com/callofpepetez" @mouseover="playClick" target="_blank" rel="noopener nofollow" class="hover:scale-150 hover:skew-x-12">
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                    viewBox="0 4 50 38" class="w-7 h-7">
                    <path
                        d="M 50.0625 10.4375 C 48.214844 11.257813 46.234375 11.808594 44.152344 12.058594 C 46.277344 10.785156 47.910156 8.769531 48.675781 6.371094 C 46.691406 7.546875 44.484375 8.402344 42.144531 8.863281 C 40.269531 6.863281 37.597656 5.617188 34.640625 5.617188 C 28.960938 5.617188 24.355469 10.21875 24.355469 15.898438 C 24.355469 16.703125 24.449219 17.488281 24.625 18.242188 C 16.078125 17.8125 8.503906 13.71875 3.429688 7.496094 C 2.542969 9.019531 2.039063 10.785156 2.039063 12.667969 C 2.039063 16.234375 3.851563 19.382813 6.613281 21.230469 C 4.925781 21.175781 3.339844 20.710938 1.953125 19.941406 C 1.953125 19.984375 1.953125 20.027344 1.953125 20.070313 C 1.953125 25.054688 5.5 29.207031 10.199219 30.15625 C 9.339844 30.390625 8.429688 30.515625 7.492188 30.515625 C 6.828125 30.515625 6.183594 30.453125 5.554688 30.328125 C 6.867188 34.410156 10.664063 37.390625 15.160156 37.472656 C 11.644531 40.230469 7.210938 41.871094 2.390625 41.871094 C 1.558594 41.871094 0.742188 41.824219 -0.0585938 41.726563 C 4.488281 44.648438 9.894531 46.347656 15.703125 46.347656 C 34.617188 46.347656 44.960938 30.679688 44.960938 17.09375 C 44.960938 16.648438 44.949219 16.199219 44.933594 15.761719 C 46.941406 14.3125 48.683594 12.5 50.0625 10.4375 Z">
                    </path>
                </svg>
            </a>
            <a href="https://discord.gg/2CPeSRqKvG" @mouseover="playClick" target="_blank" rel="noopener nofollow" class="hover:scale-150 hover:skew-x-12">
                <svg
                    xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 2 50 38"
                    class="w-7 h-7">
                    <path
                        d="M42.298,11.65c-0.676-1.021-1.633-1.802-2.768-2.256c-2.464-0.988-4.583-1.648-6.479-2.02 C31.721,7.114,30.404,7.768,29.771,9l-0.158,0.308c-1.404-0.155-2.895-0.207-4.593-0.164c-1.741-0.042-3.237,0.009-4.643,0.164 L20.22,9c-0.633-1.232-1.952-1.885-3.279-1.625c-1.896,0.371-4.016,1.031-6.479,2.02c-1.134,0.454-2.091,1.234-2.768,2.256 c-4.721,7.131-6.571,14.823-5.655,23.517c0.032,0.305,0.202,0.578,0.461,0.741c3.632,2.29,6.775,3.858,9.891,4.936 c1.303,0.455,2.748-0.054,3.517-1.229l1.371-2.101c-1.092-0.412-2.158-0.9-3.18-1.483c-0.479-0.273-0.646-0.884-0.373-1.363 c0.273-0.481,0.884-0.65,1.364-0.373c3.041,1.734,6.479,2.651,9.942,2.651s6.901-0.917,9.942-2.651 c0.479-0.277,1.09-0.108,1.364,0.373c0.273,0.479,0.106,1.09-0.373,1.363c-1.056,0.603-2.16,1.105-3.291,1.524l1.411,2.102 c0.581,0.865,1.54,1.357,2.528,1.357c0.322,0,0.647-0.053,0.963-0.161c3.125-1.079,6.274-2.649,9.914-4.944 c0.259-0.163,0.429-0.437,0.461-0.741C48.869,26.474,47.019,18.781,42.298,11.65z M18.608,28.983c-1.926,0-3.511-2.029-3.511-4.495 c0-2.466,1.585-4.495,3.511-4.495s3.511,2.029,3.511,4.495C22.119,26.954,20.534,28.983,18.608,28.983z M31.601,28.957 c-1.908,0-3.478-2.041-3.478-4.522s1.57-4.522,3.478-4.522c1.908,0,3.478,2.041,3.478,4.522S33.509,28.957,31.601,28.957z">
                    </path>
                </svg>
            </a>

        <audio autoplay loop source src="../../br1.wav" type="audio/mpeg"></audio>     
        </div>  
    </div>
</template>
    
<script setup>
    import { RouterLink } from "vue-router"

    const audio = new Audio('../../click.mp3');

    function playClick() {
        audio.play();
        console.log(audio);
    }
</script>

